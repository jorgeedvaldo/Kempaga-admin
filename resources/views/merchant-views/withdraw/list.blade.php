...
@section('content')
    <div class="content container-fluid">
        <div class="d-flex align-items-center gap-3 mb-3">
            <img width="24" src="{{ dynamicAsset(path: 'public/assets/admin/img/media/cash-withdrawal.png') }}"
                alt="{{ translate('image') }}">
        ...
        <div class="card">
                        ...
                        <ul class="dropdown-menu">
                            <li>
                                <a class="dropdown-item d-flex align-items-center gap-2"
                                    ...
                                    <img width="20"
                                        src="{{ dynamicAsset(path: 'public/assets/admin/img/media/excel.png') }}"
                                        alt="{{ translate('image') }}">
                        ...
                        <select name="withdrawal_method" class="form-control js-select2-custom" id="withdrawal_method"
                            ...
                            @foreach ($withdrawalMethods as $withdrawalMethod)
                                <option value="{{ $withdrawalMethod->id }}"
                                    {{ $method == $withdrawalMethod->id ? 'selected' : '' }}>
                                    {{ translate($withdrawalMethod->method_name) }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="table-responsive datatable-custom">
                <table class="table table-borderless table-nowrap table-align-middle card-table table-striped">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($withdrawRequest->withdrawal_method)
                                        <span
                                            class="badge badge-pill">{{ translate($withdrawRequest->withdrawal_method->method_name) }}</span>
                                    @else
...
@push('script_2')
    <script>
        ...
        $("#withdrawal_method").on('change', function() {
            ...
            const requestStatus = urlParams.get('request_status') ?? 'all';
            const search = urlParams.get('search') ?? '';

            const withdrawalMethod = $(this).val();

            // Redirect to the new URL
            location.href = "{{ route('merchant.withdraw.list') }}" +
                '?request_status=' + encodeURIComponent(requestStatus) +
                '&withdrawal_method=' + encodeURIComponent(withdrawalMethod) +
