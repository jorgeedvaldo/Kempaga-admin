...
if (!addon_published_status('Gateways')) {
    Route::group(['prefix' => 'payment'], function () {
        ...
        Route::group(['prefix' => 'phonepe', 'as' => 'phonepe.'], function () {
            ...
            Route::any('redirect', [PhonepeController::class, 'redirect'])->name('redirect')->withoutMiddleware([VerifyCsrfToken::class]);
        });

        Route::group(['prefix' => 'cashfree', 'as' => 'cashfree.'], function () {
            Route::any('pay', [CashFreePaymentController::class, 'payment'])->name('pay');
            Route::any('callback', [CashFreePaymentController::class, 'callback'])->name('payment')->withoutMiddleware([VerifyCsrfToken::class]);
        });

        Route::group(['prefix' => 'instamojo', 'as' => 'instamojo.'], function () {
            Route::any('pay', [InstamojoPaymentController::class, 'payment'])->name('pay');
            Route::any('callback', [InstamojoPaymentController::class, 'callback'])->name('payment')->withoutMiddleware([VerifyCsrfToken::class]);
        });
